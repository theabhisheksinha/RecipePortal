<%@ Page Language="C#" AutoEventWireup="true" EnableViewState="false" CodeFile="membershipmanagers.aspx.cs" Inherits="membershipmanagers" %>
...
<div style="margin-left: 5px; margin-right: 15px;">
    ...
    <ItemTemplate>
      <tr onmouseover="this.className='rowhoover';" onmouseout="this.className='row<%# Container.ItemIndex % 2 %>';" class="row<%# Container.ItemIndex % 2 %>">
        <td width="12%" class="tdgrid" align="left" valign="middle">&nbsp;<img src="../images/user_user1.png" />&nbsp;<a class="thickbox content2" onmouseover="Tip('Click to view the complete profile.<br><br><b>-- Login Password --</b><br><b>Login Password: <%# DecryptPass(Eval("Password")) %></b><br><br><b>-- Partial User Info --</b><br><b>Full Name:</b> <%# Eval("FirstName") %> <%# Eval("LastName") %><br><b>User Role:</b> <%# Eval("UserRole") %><br><b>Date Joined: </b><%# Eval("DateJoined", "{0:M/d/yyyy}")%><br><b>Profile Last Updated On: </b><%# Eval("LastUpdated", "{0:M/d/yyyy}")%><br><b>City: </b><%# Eval("City") %><br><b>State: </b><%# Eval("State") %><br><b>Country:</b> <%# Eval("Country") %><br><br><b>-- Features Settings --</b><br><b>Show FriendsList in Profile:</b> <%# IsShowInProfile(Eval("ShowFriendsListinProfile")) %><br><b>Show CookBook in Profile: </b><%# IsShowInProfile(Eval("ShowCookBookinProfile")) %><br><b>Number of Friends Show in Profile: </b><%# Eval("NumRecordsFriendsList") %><br><b>Number of Recipes show in Profile: </b><%# Eval("NumRecordsCookBookQuickView") %><br><b>Choose Preferred Layout: </b><%# IsShowInProfile(Eval("IsUserChoosePreferredLayout")) %><br><b>Preferred Layout: </b><%# GetPreferredLayout(Eval("PreferredLayout")) %><br><b>Preferred Page Size: </b><%# Eval("PreferredPageSize") %><br><b>Receive Private Message: </b><%# IsShowInProfile(Eval("ReceivePM")) %><br><b>Receive PM Email Alert: </b><%# IsShowInProfile(Eval("PMEmailNotification")) %>', BGCOLOR, '#FFFBE1', BORDERCOLOR, '#acc6db')" onmouseout="UnTip()" href='../popupuserprofile.aspx?uid=<%# Eval("UID") %>&keepThis=true&TB_iframe=true&height=600&width=850'><%# Eval("Username")%></a></td>
        ...
        <td width="8%" class="tdgrid" align="left" valign="middle">&nbsp;&nbsp;&nbsp;&nbsp;<%# Eval("DateJoined", "{0:M/d/yyyy}")%></td>
        <td width="8%" class="tdgrid" align="left" valign="middle">&nbsp;&nbsp;&nbsp;<%# Eval("LastVisit", "{0:M/d/yyyy}")%></td>
        <td width="20%" class="tdgrid" align="left" valign="middle">&nbsp;&nbsp;<%# Eval("Email")%></td>      
