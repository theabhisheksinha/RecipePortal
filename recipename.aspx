<%@ Page Language="C#" MasterPageFile="~/SiteTemplate.master" EnableViewState="false" AutoEventWireup="true" CodeFile="recipename.aspx.cs" Inherits="recipealphaletter" Title="Untitled Page" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" Runat="Server">
    ...
    <option value="recipename.aspx?letter=<%=psLetter%>&ob=<%=psOrderBy%>&sb=<%=psSortBy%>&page=<%=psPageIndex%>&ps=<%=psPageSize%>&layout=1" >Rows</option>
    <option value="recipename.aspx?letter=<%=psLetter%>&ob=<%=psOrderBy%>&sb=<%=psSortBy%>&page=<%=psPageIndex%>&ps=<%=psPageSize%>&layout=2">Grid 2 Columns</option>
    <option value="recipename.aspx?letter=<%=psLetter%>&ob=<%=psOrderBy%>&sb=<%=psSortBy%>&page=<%=psPageIndex%>&ps=<%=psPageSize%>&layout=3">Grid 3 Columns</option>
    </select>
    <select name="url" class="ddlps" onchange="javascript:document.location = options[selectedIndex].value">
    <option selected>
    <%=psPageSize + " Records Page"%>
    </option>
    <option value="recipename.aspx?letter=<%=psLetter%>&ob=<%=psOrderBy%>&sb=<%=psSortBy%>&page=<%=psPageIndex%>&ps=10&layout=<%=pLayout%>">10 Records Page</option>
    <option value="recipename.aspx?letter=<%=psLetter%>&ob=<%=psOrderBy%>&sb=<%=psSortBy%>&page=<%=psPageIndex%>&ps=20&layout=<%=pLayout%>">20 Records Page</option>
    <option value="recipename.aspx?letter=<%=psLetter%>&ob=<%=psOrderBy%>&sb=<%=psSortBy%>&page=<%=psPageIndex%>&ps=30&layout=<%=pLayout%>">30 Records Page</option>
    <option value="recipename.aspx?letter=<%=psLetter%>&ob=<%=psOrderBy%>&sb=<%=psSortBy%>&page=<%=psPageIndex%>&ps=40&layout=<%=pLayout%>">40 Records Page</option>
    <option value="recipename.aspx?letter=<%=psLetter%>&ob=<%=psOrderBy%>&sb=<%=psSortBy%>&page=<%=psPageIndex%>&ps=50&layout=<%=pLayout%>">50 Records Page</option>
    </select>
    </div>
