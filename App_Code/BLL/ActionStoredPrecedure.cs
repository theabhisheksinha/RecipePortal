...
namespace XDRecipe.BL
{
    ...
    public sealed class Blogic
   {
      ...
      /// <returns></returns>
       ...
       /// </summary>
       /// <returns></returns>
        public IDataReader GetRecipeDetail(int ID, int Approved)
        {
            ...
            // for test, don't forget to delete test
            //return DataAccess.GetFromReader("spGetRecipeDetail", prmID, prmApproved);
            return DataAccess.GetFromReader("SELECT * FROM Recipes WITH (NOLOCK) WHERE (@prmApproved IS NOT NULL AND (LINK_APPROVED = 1 AND ID = @prmID)) OR  (@prmApproved IS NULL AND ID = @prmApproved)", prmID, prmApproved);
        }
        ...
        public IDataReader GetArticleDetail(int ID, int Show)
       {
           ...
           prmShow.Value = Show;
						...
						// don't forget to delete test...
           //return DataAccess.GetFromReader("GetArticleDetails", prmID, prmShow);
            return DataAccess.GetFromReader("SELECT * FROM dbo.Cooking_Article a WITH (NOLOCK)  INNER JOIN 	dbo.Cooking_Article_Category b WITH (NOLOCK)  ON a.CAT_ID = b.CAT_ID WHERE (2 IS NOT NULL AND (a.Show = 1 AND a.ID = 2)) OR @Show_Val IS NULL", prmID, prmApproved);
       }
