<%@ Page Language="C#" MasterPageFile="~/SiteTemplate2.master" EnableViewState="false" AutoEventWireup="true" CodeFile="findallarticlebyauthor.aspx.cs" Inherits="findallarticlebyauthor" Title="Untitled Page" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" Runat="Server">
    ...
    By&nbsp;<img src="images/user-icon.gif" />&nbsp;<a href="userprofile.aspx?uid=<%# Eval("UID") %>" onmouseover="Tip('View <b><%# Eval("Author") %></b> profile.', BGCOLOR, '#FFFBE1', BORDERCOLOR, '#acc6db')" onmouseout="UnTip()"><%# Eval("Author") %></a>&nbsp;&nbsp;<asp:HyperLink ID="editlink" CssClass="content2" Visible="false" runat="server" EnableViewState="false" />&nbsp;&nbsp;Published on <span class="cyel"><%# CustomDateFormat(Eval("Date"))%></span>&nbsp;&nbsp;Hits: <span class="cmaron3"><%# Eval("Hits", "{0:#,###}")%></span>&nbsp;&nbsp;Rating:&nbsp;<img src="images/<%# Eval("Rating", "{0:0.0}")%>.gif" />&nbsp;(<span class="cgr"><%# Eval("Rating", "{0:0.0}")%></span>) votes <span class="cyel"><%# Eval("NoRates")%></span>
    </span>
    </div>
    </div>
    </div>
